...
test('local CLI commands never start a task, but /compact marks the active task complete', () => {
  assert.deepEqual(parseSignal('Claude Code', {type:'user',uuid:'a',message:{content:'/compact'}}), {kind:'compact'})
  assert.deepEqual(parseSignal('Claude Code', {type:'user',uuid:'b',message:{content:'<command-name>/compact</command-name>'}}), {kind:'compact'})
  ...
  assert.deepEqual(parseSignal('Claude Code', {type:'user',uuid:'d',isCompactSummary:true,message:{content:'This session is being continued…'}}), {kind:'compact'})
  assert.equal(parseSignal('Claude Code', {type:'user',uuid:'e',message:{content:'/clear'}}), null)
  assert.equal(parseSignal('Claude Code', {type:'user',uuid:'f',message:{content:'执行任务'}}).kind, 'start')
})
test('a compact signal completes an active task instead of leaving a phantom running task', () => {
  const t = new TaskTracker()
  t.accept('a','Claude Code',{kind:'start',id:'1'},0)
  t.accept('a','Claude Code',{kind:'compact'},1000)
  assert.equal(t.snapshot(2000).state,'completed')
...
test('tool calls yield a human-readable activity step', () => {
  ...
  assert.equal(bash.activity, 'Bash: run the test suite')
  const read = parseSignal('Claude Code', {type:'assistant', message:{stop_reason:'tool_use', content:[{type:'tool_use', name:'Read', input:{file_path:'/repo/src/main/agentMonitor.ts'}}]}})
  assert.equal(read.activity, 'Read agentMonitor.ts')
  const edit = parseSignal('Claude Code', {type:'assistant', message:{stop_reason:'tool_use', content:[{type:'tool_use', name:'Edit', input:{file_path:'/repo/src/shared/types.ts'}}]}})
  assert.equal(edit.activity, 'Edit types.ts')
  const exec = parseSignal('Codex', {type:'response_item', payload:{type:'function_call', name:'exec', arguments:'text(await tools.exec_command({cmd:"npm test",max_output_tokens:3000}));'}})
  assert.equal(exec.activity, 'exec: npm test')
})
test('the tracker surfaces the latest activity step', () => {
  const t = new TaskTracker()
  t.accept('a','Claude Code',{kind:'start',id:'1'},0)
  t.accept('a','Claude Code',{kind:'activity',stage:2,activity:'Read agentMonitor.ts'},1000)
  assert.equal(t.snapshot(1000).activity, 'Read agentMonitor.ts')
  t.accept('a','Claude Code',{kind:'activity',stage:2,activity:'Edit types.ts'},2000)
  assert.equal(t.snapshot(2000).activity, 'Edit types.ts')
})
